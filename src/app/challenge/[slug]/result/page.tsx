...
export default function ChallengeResultPage({ params }: ChallengePageProps) {
  ...
  const [attempt, setAttempt] = useState<{ finishedAt: Date; startedAt: Date } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/challenge/result/get", {
      ...
      .then((data) => {
        if (data.success) {
          console.log(data);
          setAttempt({ startedAt: data.attempt.startedAt, finishedAt: data.attempt.finishedAt })
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <span>{((new Date(attempt.finishedAt).getTime() - new Date(attempt.startedAt).getTime()) / 1000).toFixed(0)}s</span>
        </div>
      </div>}
      {!seeMore && (
